...
contract RelayHub is RelayHubApi {
    ...
    function canRelay(address relay, address from, RelayRecipient to, bytes memory encodedFunction, uint transactionFee, uint gasPrice, uint gasLimit, uint nonce, bytes memory approval) public view returns(uint32) {
    ...
    function handleAcceptRelayCall(RelayRecipient to, bytes memory acceptRelayedCallRawTx) private view returns (uint32){
        bool success;
    ...
    function relay(address from, address to, bytes memory encodedFunction, uint transactionFee, uint gasPrice, uint gasLimit, uint nonce, bytes memory approval) public {
        ...
        uint32 canRelayResult = canRelay(msg.sender, from, RelayRecipient(to), encodedFunction, transactionFee, gasPrice, gasLimit, nonce, approval);
        if (canRelayResult != 0) {
    ...
    function recipientCalls(address from, address to, address relayAddr, bytes calldata encodedFunction, uint transactionFee, uint gasLimit, uint initialGas) external returns (bool) {
        ...
        transaction = abi.encodeWithSelector(RelayRecipient(to).postRelayedCall.selector, relayAddr, from, encodedFunction, success, (gasOverhead + initialGas -gasleft()), transactionFee);
