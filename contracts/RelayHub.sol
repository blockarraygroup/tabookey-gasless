...
contract RelayHub is RelayHubApi {
    ...
    function canRelay(address relay, address from, RelayRecipient to, bytes memory encodedFunction, uint transactionFee, uint gasPrice, uint gasLimit, uint nonce, bytes memory approval) public view returns(uint) {
    ...
    function handleAcceptRelayCall(RelayRecipient to, bytes memory acceptRelayedCallRawTx) private view returns (uint){
        bool success;
    ...
    function relay(address from, address to, bytes memory encodedFunction, uint transactionFee, uint gasPrice, uint gasLimit, uint nonce, bytes memory approval) public {
        ...
        uint canRelayResult = canRelay(msg.sender, from, RelayRecipient(to), encodedFunction, transactionFee, gasPrice, gasLimit, nonce, approval);
        if (canRelayResult != 0) {
